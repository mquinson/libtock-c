...
// TODO this is ugly but scope should be fine right?
...
static void putstr_upcall(int   _x __attribute__ ((unused)),
  ...
  if (putstr_head == NULL) {
    ...
    int ret;
    ret = modem_putnstr_async(putstr_head->buf, putstr_head->len, putstr_upcall, NULL);
    if (ret < 0) {
...
int modem_putnstr(const char *str, size_t len) {
  ...
  if (putstr_tail == NULL) {
    ...
    ret = modem_putnstr_async(data->buf, data->len, putstr_upcall, NULL);
...
#pragma GCC diagnostic pop
  ...
  allow_ro_return_t ro = allow_readonly(DRIVER_NUM_CONSOLE_MODEM, 1, str, len);
  if (!ro.success) {
    return tock_status_to_returncode(ro.status);
  }

  subscribe_return_t sub = subscribe(DRIVER_NUM_CONSOLE_MODEM, 1, cb, userdata);
  if (!sub.success) {
    return tock_status_to_returncode(sub.status);
  }

  syscall_return_t com = command(DRIVER_NUM_CONSOLE_MODEM, 1, len, 0);
  return tock_command_return_novalue_to_returncode(com);
}

int modem_getnstr_async(char *buf, size_t len, subscribe_upcall cb, void* userdata) {
  allow_rw_return_t rw = allow_readwrite(DRIVER_NUM_CONSOLE_MODEM, 1, buf, len);
  if (!rw.success) {
    return tock_status_to_returncode(rw.status);
  }

  subscribe_return_t sub = subscribe(DRIVER_NUM_CONSOLE_MODEM, 2, cb, userdata);
  if (!sub.success) {
    return tock_status_to_returncode(sub.status);
  }

  syscall_return_t com = command(DRIVER_NUM_CONSOLE_MODEM, 2, len, 0);
  return tock_command_return_novalue_to_returncode(com);
}
...
int modem_getnstr(char *str, size_t len) {
  ...
  ret = modem_getnstr_async(str, len, getnstr_upcall, NULL);
  if (ret < 0) {
    return ret;
  }
...
int modem_getch(void) {
  ...
  r = modem_getnstr(buf, 1);
  return (r == RETURNCODE_SUCCESS) ? buf[0] : RETURNCODE_FAIL;
}

int modem_getnstr_abort(void) {
  syscall_return_t com = command(DRIVER_NUM_CONSOLE_MODEM, 3, 0, 0);
  return tock_command_return_novalue_to_returncode(com);
}
